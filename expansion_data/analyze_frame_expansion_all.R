...
for(file in datafiles) {
  ...
  theoretical <- data.frame(frame_t = data_filtered$frame_t) %>%
    mutate(comp_z = calc_z_comp(frame_t, frame_t[1], frame_z, 236E-6),
  ...
  ggplotRegression(fit) +
    ...
    # coord_equal() +
    scale_x_continuous(breaks=seq(-0.5,0.5,0.02))+
  ...
  ggplot(data=filter(data_all_wide),
         aes(elapsed_time_mean)) +
    geom_hline(yintercept = 0, linetype=2) +
